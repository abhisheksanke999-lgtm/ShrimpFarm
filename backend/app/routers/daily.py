...
from app.schemas import DailyCreate, DailyOut, MessageOut
...
def _own_pond(db: Session, user_id: int, pond_name: str) -> bool:
    return (
        db.query(Pond)
        .filter(Pond.user_id == user_id, Pond.name == pond_name)
        .first()
        is not None
    )
...
def list_daily(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return (
...
def create_daily(payload: DailyCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    pond_name = payload.pond_name.strip()
    if not pond_name or not payload.log_date:
        raise HTTPException(status_code=400, detail="Please select a pond and date.")
